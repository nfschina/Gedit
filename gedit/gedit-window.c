	...
	else
	{
		if (dirname != NULL)
		{
			main_title = g_strdup_printf ("%s (%s) - gedit",
			                              ...
			                              dirname);
			subtitle = dirname;
		        title = g_strdup_printf("%s (%s/)", name, subtitle);
		}
		else
		{
                               ...
                               GeditWindow *window)
{
	...
	if (visible)
	{
		...
		if (tokens)
		{
			...
			gtk_header_bar_set_decoration_layout (GTK_HEADER_BAR (window->priv->side_headerbar), tokens[0]);  // KQ
