		...
		gboolean                 create)
{
	...
	char* filename = g_file_get_parse_name(file);
    	...
    	if((fp2 = fopen("/tmp/copyfile","w")) == NULL|| (fp = fopen(filename,"r")) == NULL){
        	perror("fail to write/read!");
		exit(1);
    	}
	...
	sprintf(bash_command, "mv /tmp/copyfile '%s'", filename);
   	if(val > 0){
        	system(bash_command);
    	}
    	if(val==0){
        	system("rm /tmp/copyfile");
    	}
	g_free(filename);
			 ...
			 GTask                  *task)
{
	...
	gchar *filename = g_file_get_basename (location);
		   ...
		   gpointer             user_data)
{
	...
	GFile *gfile = gtk_source_file_get_location (file);
