...
static void change_label_propertys (GtkWidget *lbl, gpointer data) {
    if (GTK_IS_LABEL(lbl)) {
        gtk_label_set_line_wrap(GTK_LABEL(lbl), TRUE);
        gtk_label_set_line_wrap_mode(GTK_LABEL(lbl), PANGO_WRAP_CHAR);
        gtk_label_set_max_width_chars(GTK_LABEL(lbl), 72);
    }
}
...
build_single_doc_dialog (GeditCloseConfirmationDialog *dlg)
{
	...
	GtkWidget* content_area = gtk_message_dialog_get_message_area(GTK_MESSAGE_DIALOG(dlg));
	gtk_container_forall(GTK_CONTAINER(content_area), change_label_propertys, NULL);
